...
exports.handler = async (event) => {
  ...
  let messages = [
    ...
    ...validatedChatHistory, // 将之前的聊天历史添加到 messages 数组中
  ...
  // 将机器人的回应添加到聊天历史中
  validatedChatHistory.push({role: "assistant", content: data.choices[0].message.content.trim()});

  return {
    statusCode: 200,
    body: JSON.stringify({
      message: data.choices[0].message.content.trim(),
      updatedChatHistory: validatedChatHistory // 返回更新后的聊天历史，以便客户端存储
    })
  };
}
